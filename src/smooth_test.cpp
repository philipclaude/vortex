...
double energy(VoronoiDiagram &voronoi, int t_type, int order) {
  double energy = 0.0;
  Topology<Polygon> &polygons = voronoi.polygons();
...
double energy_objective(unsigned n, const double *sites, double *grad,
  ...
  energy_data<SquareDomain> &data_e =
      *static_cast<energy_data<SquareDomain> *>(ed);
...
UT_TEST_CASE(optimized_points_square) {
  ...
  double first_e = energy(voronoi, 0, 4);

  // fill optimization data
  energy_data<SquareDomain> data_e = {first_e,  voronoi, domain, options,
                                      vertices, dim,     0};
  int n = sites.size();
